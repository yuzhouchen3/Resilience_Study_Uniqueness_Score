...
path = '/Users/yuzhouchen/iCloud Drive (Archive)/Documents/NSF_INTERN_2020/PDS_dataset'
...
target_substation = 51 # tune substation
expansion_graph = expansion_plan1_graph

uniqueness_score_mat = np.zeros(shape=(50,1), dtype= np.float32)
for target_load in np.arange(1,51):
    tmp_load_tmp_sub = nx.all_simple_paths(expansion_graph, source=target_load, target=target_substation)
    paths_tmp_load_tmp_sub = list(tmp_load_tmp_sub)
    pairwise_paths = [list(path) for path in
                      map(nx.utils.pairwise, paths_tmp_load_tmp_sub)]  # get pairwise path between "from" node and "to" node
    ...
    ordered_lists_tmp_load_tmp_sub = list(itertools.chain.from_iterable(ordered_paths_tmp_load_tmp_sub.tolist())) # sorted lists
    ...
    for uu in range(len(paths_tmp_load_tmp_sub)):
        if uu == 0:
            ...
            if (target_load in tmp_intersect) & (target_substation in tmp_intersect):
